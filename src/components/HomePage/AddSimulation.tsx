...
import { LdTypo, LdInputMessage, LdModal, LdLabel, LdOption, LdInput, LdSelect, LdButton, LdLoading } from "@emdgroup-liquid/liquid/dist/react";
...
const AddNewSimulation: React.FC<AddNewSimulationProps> = ({ onClose, onAddSimulation,loadingProgressBar }) => {
    const modalRef = useRef(null); // Reference for the modal
    const [selectedCycle, setSelectedCycle] = useState<CycleKey[]>([]); // State for plant data
    const { formState, handleSubmit, register, setValue, reset, getValues } = useForm<SimulationFormData>({
        mode: 'onTouched',
    ...
    const getPlanDetails = async () => {
        try {
            ...
            if (res.data) {
                return ["SITE_NANTONG","SITE_RIO", "SITE_BARI" ,"SITE_ATUSA" , "SITE_AUBONNE", "SITE_MOLLET"];
            } else {
                display_notification('warn', 'No active site returned');
                return []; // Return an empty array if no data
            }
        } catch (err) {
            display_notification('alert', 'An error occurred while fetching active site data');
            return []; // Return an empty array in case of an error
        }
    };

    const getPlantScenarios = useCallback((SelectedPlant: string) => {
        // Check if SelectedPlant is an empty string or not
        if (!SelectedPlant) {
            display_notification('warn', 'Please select a plant before fetching scenarios.');
        ...
        axios
            ...
            .then((res) => {
                if (res.data) {
                    ...
                    if (res.data.length > 0) {
                        setValue('cycle_key', res.data[0].cycle_key, { shouldValidate: true });
                        setValue('by_scenario', res.data[0].by_scenario, { shouldValidate: true });
                        setValue('cutoff_date', res.data[0].cutoff_date, { shouldValidate: true });
                    }
                } else {
            ...
            .catch((err) => {
                display_notification('alert', 'An error occurred while fetching scenarios data');
            });
        setDisabledAction(false);
    }, [setValue]); // Include setValue in the dependencies array
    ...
    const handleCycleChange = (e: any) => {
        ...
        const selectedCycleKey = selectedCycle.find(cycle => cycle.cycle_key === selectedKey);
        if (selectedCycleKey) {
            setValue('cycle_key', selectedCycleKey.cycle_key, { shouldValidate: true });
            setValue('by_scenario', selectedCycleKey.by_scenario, { shouldValidate: true });
            setValue('cutoff_date', selectedCycleKey.cutoff_date, { shouldValidate: true });
    ...
    const handleFormSubmit: SubmitHandler<SimulationFormData> = async (simulationData: any) => {
        onClose();
        loadingProgressBar(true);
        display_notification('warn', `Creating a new simulation. Please wait and do not close or refresh the page. You will be notified once the simulation is created.`, 20000);
        ...
        try {
            ...
            if (res.data.simulation_id) {
                ...
                display_notification('warn', 'Simulation master did not respond to your request.');
            }
        } catch (err) {
            display_notification('alert', 'Your request to add to the simulation has failed: ' + err);
        }
    ...
    return (
        <form autoComplete="off" data-testid="form" name="simulation_add_form" onSubmit={handleSubmit(handleFormSubmit, handleFormInvalid)}>
            <LdModal
            ...
            >
                <LdTypo variant="h4" slot="header">Create new Simulation</LdTypo>
                <div className="space-y-2"> {/* Padding and spacing between elements */}
                    {/* Multiple Textbox Section */}
                    <div className="grid grid-cols-2 gap-4"> {/* Two-column layout */}
                        <div>
                            <LdLabel size="m">SIMULATION NAME</LdLabel>
                            <LdInput
                                className="w-full"
                            ...
                            />
                            {errors.simulation_name && <LdInputMessage>{errors.simulation_name.message}</LdInputMessage>}
                        </div>
                        <div>
                            ...
                            {errors.plant && <LdInputMessage>{errors.plant.message}</LdInputMessage>}
                        </div>
                    </div>
                    <div>
                        ...
                        />
                        {errors.simulation_description && <LdInputMessage>{errors.simulation_description.message}</LdInputMessage>}
                    </div>
                    <div>
                        <LdLabel size="m">CYCLE DETAIL KEY</LdLabel>
                         {disabledAction  ? <LdLoading /> : '' }
                        <select
                            className="w-full custom-select"
                            {...register('cycle_key', { required: 'Please select cycle key.' })}
                        ...
                        >
                            ...
                            {selectedCycle.map((cycle_Object) => (
                                <option key={cycle_Object.cycle_key} selected={cycle_Object.cycle_key === getValues('cycle_key')} value={cycle_Object.cycle_key}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <LdInput disabled value={getValues('by_scenario') || ""} placeholder="CUTOFF DATE" type="text" className="w-full" />
                        </div>
                        <div>
                            <LdLabel size="m">CUTOFF DATE</LdLabel>
                            <LdInput disabled placeholder={getValues('cutoff_date') || "CUTOFF DATE"} type="text" className="w-full" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="flex space-x-4">
                                <LdInput
                                    {...register('initial_date', { required: 'Please select an initial date.' })}
                                    type='date'
                                    className="w-full"
                                ...
                                <LdInput
                                    {...register('end_date', {
                                        ...
                                        validate: value => {
                                            ...
                                            return endDate > startDate || 'End date must be after the initial date.';
                                        }
                                    })}
                                    type='date'
                                    className="w-full"
                            ...
                            {errors.initial_date && <LdInputMessage>{errors.initial_date.message}</LdInputMessage>}
                            {errors.end_date && <LdInputMessage>{errors.end_date.message}</LdInputMessage>}
                        </div>
                    </div>
                </div>
                
                <div className="flex justify-end space-x-2 p-4"> 
                    <LdButton disabled={Object.keys(errors).length > 0} slot="footer" type='submit' className="bg-blue-500 text-white">Submit</LdButton>
                    <LdButton slot="footer" onClick={() => { closeModalSimulation() }} mode="secondary" type="button">Cancel</LdButton>
                </div>
            </LdModal>
        </form>
    );
