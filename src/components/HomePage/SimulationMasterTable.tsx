...
import { LdNotification,LdButton, LdCard, LdIcon, LdLoading, LdProgress, LdTable, LdTableBody, LdTableCell, LdTableHead, LdTableHeader, LdTableRow, LdModal, LdTypo,LdTableToolbar,LdTableCaption } from "@emdgroup-liquid/liquid/dist/react";
...
const simulationMasterRid = "ri.phonograph2.main.table.a0f7ae81-fa7c-4d4d-88e4-51e7387dc8a8"
...
const SimulationMasterTable: React.FC = () => {
    ...
    const getAllSimulation = useCallback(() => {
        ...
        axios
            ...
            .catch((err) => {
                display_notification('alert','An error ocurred while fetching simulations data');
            })
    }, [simulationMasterRid]);
    ...
    const handleOpenCompareSimulationView = () => {
        ...
        if (selectedRows && selectedRows.length === 2) {
            ...
            const allSuccess = selectedSimulation1.status === 'SUCCESS' && selectedSimulation2.status === 'SUCCESS';
            const samePlant = selectedSimulation1.plant === selectedSimulation2.plant;
    
            if (allSuccess && samePlant) {
                const newUrl = `comparative/${selectedRows.join('/')}/${selectedSimulation1.plant}`;
                navigate(newUrl); 
            } else {
                display_notification('alert', "Please select simulations that have a status of 'SUCCESS' and belong to the same PLANT.");
            }
        } else {
            display_notification('warn', "Please select two simulations for comparison.");
        }
    };
        ...
        if (simulationId){
            ...
            .then((response) => {
                if (response.status === 200){
                    ...
                    setElements(prevElements => prevElements.filter(x => x.simulation_id !== simulationId));
                    display_notification('info', "Succesfully deleted simulation");
                }
            })
            .catch((error) => {
                display_notification('alert', "Please try agin we are unable to delete the simulation");
            });
        }
    };

    const onSelect = (e: CustomEvent<{rowIndex: number; selected: boolean}>) => {
    ...
    const handleAddSimulation = (NewSimulation:any) => {
        ...
        display_notification('info', `A new simulation has been added to the list: <b>${NewSimulation.simulation_name}</b>`);
    };

    return (
        <div className='appContainer'>
            {isLoadingProgress && (
                <LdProgress className="w-full" pending ariaValuemax={100} aria-valuetext="indeterminate" />
            )}
            <div className='slate-ui-container px-4 simulationMasterTable'>
                <div className="py-4 justify-end flex gap-3">
                    <LdButton onClick={handleOpenCompareSimulationView} disabled={isCompareSimulationsDisabled} size="sm" mode="highlight">
                        <LdIcon name="dashboard" size="sm"/> Compare Simulations
                    </LdButton>
                    <LdButton onClick={handleOpenSimulationView} disabled={isOpenSimulationDisabled} size="sm">
                        <LdIcon name="external-export" size="sm"/> Open Simulation
                    </LdButton>
                    <LdButton onClick={() => setConfirmDeleteOpen(true)} disabled={isOpenSimulationDisabled} size="sm" mode="danger">
                        <LdIcon name="bin" size="sm"/> Delete Simulation
                    </LdButton>
                    <LdButton onClick={openModal} className="custom-green-button" size="sm">
                        ...
                        <LdTableBody>
                            {elements.length > 0 ? (
                                elements.map((element, index) => (
                                    <LdTableRow selectable key={element.simulation_id} onLdTableSelect={onSelect}>                                    
                                        <LdTableCell className='font-medium text-sm leading-4'>{element.plant}</LdTableCell>
                                        <LdTableCell className='font-medium text-sm leading-4 word-break w-[150px]'>{element.simulation_name}</LdTableCell>
                                        <LdTableCell className='font-medium text-sm leading-4'>{element.cycle_key}</LdTableCell>
                                        <LdTableCell className='font-medium text-sm leading-4 word-break'>{element.by_scenario}</LdTableCell>
                                        <LdTableCell className='font-medium text-sm leading-4'>{element.init_date}</LdTableCell>
                                        <LdTableCell className='font-medium text-sm leading-4'>{element.end_date}</LdTableCell>
                                        <LdTableCell className='font-medium text-sm leading-4'>{element.created_by_full_name}</LdTableCell>
                                        <LdTableCell className='font-medium text-sm leading-4'>{element.modified_by_full_name}</LdTableCell>
                                        <LdTableCell className='font-medium text-sm leading-4'>{new Date(element.modified_at).toLocaleString("en-us")}</LdTableCell>
                                        <LdTableCell className='font-medium text-sm leading-4'>{element.status}</LdTableCell>
                                    </LdTableRow>
                                ))
                            ) : (
                                <LdTableRow>
            ...
            <LdModal cancelable={false} open={confirmDeleteOpen} onLdmodalclosed={() => setConfirmDeleteOpen(false)}>
                <LdTypo variant="h4" slot="header"> Confirmation Alert </LdTypo>
                <LdTypo style = {{textAlign: 'center'}}> Are you sure you want to delete the confirmed simulation?</LdTypo>
                <LdButton slot="footer" mode="danger" onClick={() => setConfirmDeleteOpen(false)}> Cancel </LdButton>
                <LdButton slot="footer" onClick={() => {
