...
import { LdTabs, LdTablist, LdTab, LdLoading } from "@emdgroup-liquid/liquid/dist/react";
...
const getAggregatedLoadData = (outputData: LoadData, originalData: LoadData, Lines: string[]) => {
    ...
    for (const [line, lineData] of Object.entries(outputData)) {
        ...
        for (const key in lineData) {
            ...
            if (["HOURS", "QTY", "BATCHES", "PALLETS", "ABSORPTION"].includes(unit) && original !== "ORIGINAL") {
                ...
                const differences: number[] = lineData[key].map((x: number, idx: number) => x - (originalData[line][checkOrignalValue][idx] || 0));
                const added: number = differences.filter((x: number) => x > 0).reduce((acc: number, val: number) => acc + val, 0);
                const reduced: number = differences.filter((x: number) => x < 0).reduce((acc: number, val: number) => acc + val, 0);
...
const CapacitySummaryReport: React.FC<CapacitySummaryProps> = (
    ...
    const [sliderSelectedMinDate, setSliderSelectedMinDate] = useState(getFirstDayOfMonth(SimulationData.init_date));
    const [sliderSelectedMaxDate, setSliderSelectedMaxDate] = useState(getFirstDayOfMonth(SimulationData.end_date));
    const [selectedValues, setSelectedValues] = useState<"Years" | "Quarters" | "Months">("Months");
    const [selectedValues1, setSelectedValues1] = useState<"Quantity" | "Hours" | "Batches">("Hours");
    const [summaryTableData, setSummaryTableData] = useState<Rows>(defaultSummaryTableData);
    ...
    const capacityFilterDetails = useMemo(() => { 
        ...
        const capacities: CapacityData = Object.keys(ModifiedCapacityData).length > 0 
            ? ModifiedCapacityData 
            : CapacityData;
        ...
        return Object.keys(filteredCapacities).length > 0 ? filteredCapacities : CapacityData;
    ...
    const getLoadOutputAggregated = useMemo(() => {
        const data: LoadData3 = editedInputData.hasOwnProperty(Lines[0]) ? editedInputData : demanInputdata; 
        const finalData: Record<string, Record<string, number>> = {};
    ...
    const aggregatedData = () => {
        const modifiedData = editedInputData.hasOwnProperty(Lines[0]) ? editedInputData : demanInputdata;
    ...
    const getAllDates = () => {
        ...
        while (start <= endDate) { // Check if start is less than or equal to endDate
    ...
    const getDatesGroups = ()=> {
        ...
        const filtered_dates = all_dates.filter(date => date >= sliderSelectedMinDate && date <= sliderSelectedMaxDate); // Use direct comparison
    ...
    const getGraphData = () => {
        ...
        setChartData({ selectedFilter, loadGraphData, capacityGraphData , optimalGraphData });
    }

    const getLoadGraph = () => {

        const originals: Record<string, Record<string, number>> = getLoadOriginalAggregated;
        const currents: Record<string, Record<string, number>> = getLoadOutputAggregated;
        const metric: string = selectedValues1 === "Hours" ? "HOURS" : selectedValues1 === "Quantity" ? "QTY" : "BATCHES";
        ...
        for (const [group, months] of Object.entries(dateGroups)) {
            ...
            if (labelUnit === "Years") {
                ...
                const quarterStartMonth: Record<string, string> = { Q1: "01", Q2: "04", Q3: "07", Q4: "10" };
        ...
        if (labelUnit === "Months") {
            ...
            minDate = `${sliderMinYear}-${adjustToQuarterStart(parseInt(sliderMinMonth))}-01`;
            maxDate = `${sliderMaxYear}-${adjustToQuarterStart(parseInt(sliderMaxMonth))}-01`;
        } else {
            ...
            for (const [group, months] of Object.entries(dates_groups)) {
                ...
                for (const month of months) {
                    for (const line of Object.keys(capacities)) {
                        ...
                        if (index !== -1) {
                            actual_capacity += capacities[line]["PRODUCTIVE_CAPACITY_HOURS"][index];
                            min_capacity += Math.round(capacities[line]["MIN_SHIFTS"][index] * capacities[line]["HOURS_PER_SHIFT"][index] * 
                ...
                if (selectedValues === "Years") {
                    ...
                    const [year, quarter] = group.split("-");
                    const monthMap: Record<string, string> = { Q1: "01", Q2: "04", Q3: "07", Q4: "10" };
    ...
    const getSummaryTableReport = () => {
        ...
        for (const metric of metrics){
            ...
            for (const [line, line_data] of Object.entries(load_data_agg)){
                ...
                for(let i = min_date_index; i <= max_date_index; i +=1){
                    ...
                    switch (metric){
                        ...
                        case "Demand Volume (Batches)":
                            value = load_data_agg[line][`${dates[i].replace(/-/g, '_')}_BATCHES`];
                            break;
                        case "Demand Volume (Pallets)":
                            value = load_data_agg[line][`${dates[i].replace(/-/g, '_')}_PALLETS`];
                            break;
                        case "Demand Load (h)":
                            value = load_data_agg[line][`${dates[i].replace(/-/g, '_')}_HOURS`];
                            ratios_data[line]["new_hours"].push(value);
                            break;
                        case "Initial (h)":
                            value = load_data_original_agg[line][`${dates[i].replace(/-/g, '_')}_HOURS`];
                            ratios_data[line]["initial_hours"].push(value);
                            break;
                        case "Added (h)":
                            value = load_data_differences_agg[line][`${dates[i].replace(/-/g, '_')}_HOURS_ADDED`];
                            break;
                        case "Removed (h)":
                            value = load_data_differences_agg[line][`${dates[i].replace(/-/g, '_')}_HOURS_REDUCED`];
                            break;
                        case "Min Capacity (h)":
                            value = capacity_data[line]["MIN_SHIFTS"][capaDateIndex] * capacity_data[line]["HOURS_PER_SHIFT"][capaDateIndex] * capacity_data[line]["OEE"][capaDateIndex];
                            break;
                        case "Max Capacity (h)":                            
                            value = capacity_data[line]["MAX_SHIFTS"][capaDateIndex] * capacity_data[line]["HOURS_PER_SHIFT"][capaDateIndex] * capacity_data[line]["OEE"][capaDateIndex];
                            break;
                        case "Current Installed Capacity (h)":
                            value = capacity_data[line]["PRODUCTIVE_CAPACITY_HOURS"][capaDateIndex];
                            ratios_data[line]["initial_capacity"].push(value);
                            break;
                        case "Suggested Capacity (h)":
                            value = optimization_data[line]?.["OPT_CAPACITY_HOURS"]?.length === 0 
                                ? capacity_data[line]?.["PRODUCTIVE_CAPACITY_HOURS"][capaDateIndex] 
                                : optimization_data[line]?.["OPT_CAPACITY_HOURS"]?.[capaDateIndex] || 0; // Provide a fallback if undefined
                            ratios_data[line]["new_capacity"].push(value);
                            break;
                        case "Initial D/C Ratio":
                            const initial_load = load_data_original_agg[line][`${dates[i].replace(/-/g, '_')}_HOURS`];
                            const initial_capacity = capacity_data[line]["PRODUCTIVE_CAPACITY_HOURS"][capaDateIndex];
                            rows[dates[i]].push(initial_capacity === 0 ? 0 : initial_load/initial_capacity);
                            break;
                        case "Initial Absorption (€)":
                            value = load_data_original_agg[line][`${dates[i].replace(/-/g, '_')}_ABSORPTION`];
                            break;
                        case "New D/C Ratio":
                            const new_load = load_data_agg[line][`${dates[i].replace(/-/g, '_')}_HOURS`];
                            const new_capacity = optimization_data[line]?.["OPT_CAPACITY_HOURS"]?.length === 0 
                                                ? capacity_data[line]["PRODUCTIVE_CAPACITY_HOURS"]?.[capaDateIndex] 
                                                : optimization_data[line]?.["OPT_CAPACITY_HOURS"]?.[i] || 0; 

                            
                            rows[dates[i]].push(new_capacity === 0 ? 0 : new_load/new_capacity);
                            break;
                        case "New Absorption (€)":
                            value = load_data_agg[line][`${dates[i].replace(/-/g, '_')}_ABSORPTION`]
                            break;
                    }
                ...
                if (metric === "Initial D/C Ratio"){
                    const line_total_initial_capacity = ratios_data[line]["initial_capacity"].reduce((acc, val) => acc + val, 0);
                    const line_total_initial_load = ratios_data[line]["initial_hours"].reduce((acc, val) => acc + val, 0);
                    rows["TOTALS"].push(line_total_initial_capacity === 0 ? 0 : line_total_initial_load / line_total_initial_capacity);
                }
                else if (metric === "New D/C Ratio"){
                    const line_total_new_capacity = ratios_data[line]["new_capacity"].reduce((acc, val) => acc + val, 0);
                    const line_total_new_load = ratios_data[line]["new_hours"].reduce((acc, val) => acc + val, 0);
                    rows["TOTALS"].push(line_total_new_capacity === 0 ? 0 : line_total_new_load / line_total_new_capacity);
                }
                else{
            ...
            for(let i = min_date_index; i <= max_date_index; i +=1){
                if (metric === "Initial D/C Ratio"){
                    ...
                    for (let line of Object.keys(ratios_data)){
                        ...
                        total_initial_capacity_date += ratios_data[line]["initial_capacity"][i];
                    }
                    rows[dates[i]].push(total_initial_capacity_date === 0 ? 0 : total_initial_hours_date / total_initial_capacity_date);
                }
                else if (metric === "New D/C Ratio"){
                    ...
                    rows[dates[i]].push(total_new_capacity_date === 0 ? 0 : total_new_hours_date / total_new_capacity_date);
                }
                else{
            ...
            if (metric === "Initial D/C Ratio"){
                ...
                for (const [line, agg] of Object.entries(ratios_data)){
                    total_initial_hours+=agg["initial_hours"].reduce((acc, val) => acc + val, 0);
                    total_initial_capacity+=agg["initial_capacity"].reduce((acc, val) => acc + val, 0);
                }
                rows["TOTALS"].push(total_initial_capacity === 0 ? 0 : total_initial_hours / total_initial_capacity);
            }
            else if (metric === "New D/C Ratio"){
                ...
                for (const [line, agg] of Object.entries(ratios_data)){
                    ...
                    total_new_capacity+=agg["new_capacity"].reduce((acc, val) => acc + val, 0);
                }
                rows["TOTALS"].push(total_new_capacity === 0 ? 0 : total_new_hours / total_new_capacity);
        ...
        if (selectedValues !== "Months"){
            ...
            const initial_dc_ratio_indexes = rows["PROJECTION"].map((x, idx) => x ==="Initial D/C Ratio" ? idx : -1).filter(x => x !== -1)
            const initial_hours_indexes = rows["PROJECTION"].map((x, idx) => x ==="Initial (h)" ? idx : -1).filter(x => x !== -1)
            const initial_capacity_indexes = rows["PROJECTION"].map((x, idx) => x ==="Current Installed Capacity (h)" ? idx : -1).filter(x => x !== -1)
            
            const new_dc_ratio_indexes = rows["PROJECTION"].map((x, idx) => x ==="New D/C Ratio" ? idx : -1).filter(x => x !== -1)
            const new_hours_indexes = rows["PROJECTION"].map((x, idx) => x ==="Demand Load (h)" ? idx : -1).filter(x => x !== -1)
            const new_capacity_indexes = rows["PROJECTION"].map((x, idx) => x ==="Suggested Capacity (h)" ? idx : -1).filter(x => x !== -1)
            
            for (const [group, months] of Object.entries(date_groups)){
                new_rows[group] = Array(new_rows["PROJECTION"].length).fill(0)
                for (const month of months){
                    new_rows[group] = new_rows[group].map((x:number, idx:any) => x + rows[month][idx]);
                }
                initial_dc_ratio_indexes.forEach((index, pos) => {new_rows[group][index] = new_rows[group][initial_capacity_indexes[pos]] === 0 ? 0 : new_rows[group][initial_hours_indexes[pos]] / new_rows[group][initial_capacity_indexes[pos]]});
                new_dc_ratio_indexes.forEach((index, pos) => {new_rows[group][index] = new_rows[group][new_capacity_indexes[pos]] === 0 ? 0 : new_rows[group][new_hours_indexes[pos]] / new_rows[group][new_capacity_indexes[pos]]});
            }
            rows = new_rows
        }
    ...
    const getsummaryTableFilter = () => {
        ...
        const selected_projections_index = rows["PROJECTION"].map((item, index) => selectedProjections.includes(item) ? index : -1)  // Map each item to its index if it's in secondList
        .filter(index => index !== -1);


        for (let key in rows){
            rows[key] = rows[key].filter((_: unknown, index: number) => selected_projections_index.includes(index))
        }
    ...
    return (
        <div className="w-full overflow-hidden">
            <div className="flex flex-col md:flex-row">
                <div className="p-2 chartMetric md:w-1/2">
                    <div className="m-4">
                        <LdTabs>
                            <LdTablist className='flex' size="sm" rounded="all">
                                <LdTab className='shrink-0 grow' selected={selectedValues1 === "Quantity"} onClick={() => handleTabClick1("Quantity")}>Quantity</LdTab>
                                <LdTab className='shrink-0 grow' selected={selectedValues1 === "Hours"} onClick={() => handleTabClick1("Hours")}>Hours</LdTab>
                                <LdTab className='shrink-0 grow' selected={selectedValues1 === "Batches"} onClick={() => handleTabClick1("Batches")}>Batches</LdTab>
                            </LdTablist>
                        </LdTabs>
                    </div>
                    <div className="m-4">
                        <LdTabs>
                            <LdTablist className='flex' size="sm">
                                <LdTab className='shrink-0 grow' selected={selectedValues === "Months"} onClick={() => handleTabClick("Months")}>Months</LdTab>
                                <LdTab className='shrink-0 grow' selected={selectedValues === "Quarters"} onClick={() => handleTabClick("Quarters")}>Quarters</LdTab>
                                <LdTab className='shrink-0 grow' selected={selectedValues === "Years"} onClick={() => handleTabClick("Years")}>Years</LdTab>
                            </LdTablist>
                        </LdTabs>
                    </div>
                </div>
                <div className="p-2 md:w-1/2">
                    {SimulationData && (
                        <RangeSlider simulationData={SimulationData} handleSliderChange={handleSliderChange} />
                    )}
                </div>
            </div>
            <div className="w-full charResponsive">
                <div>
                    {loading ? (
                        <div><LdLoading /></div> // You can replace this with a spinner or any loading component
                    ) : (
                        <CapacitySummaryChart inputData={chartData} />
                    )}
                </div>
            </div>
            <div>
