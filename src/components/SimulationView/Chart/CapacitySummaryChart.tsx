...
const CapacitySummaryChart: React.FC<{ inputData: InputData }> = ({ inputData }) => {
    ...
    const filterChartData = (inputData: InputData) => {
        ...
        if (inputData.selectedFilter === "Hours") {
            for (let i = 0; i < inputData.capacityGraphData.dates.length; i++) {
                ...
                if (totals[key]) {
                    totals[key].max_capacity += inputData.capacityGraphData.max_capacity[i];
                    totals[key].min_capacity += inputData.capacityGraphData.min_capacity[i];
                    totals[key].productive_capacity += inputData.capacityGraphData.productive_capacity[i];
                }
            }
        }

        if (inputData.selectedFilter === "Hours" && inputData.optimalGraphData.dates.length) {
            for (let i = 0; i < inputData.optimalGraphData.dates.length; i++) {
                ...
                // Check if the date exists in totals
                if (totals[key]) {
                    totals[key].optimal_capacity += roundToDecimal(inputData.optimalGraphData.values[i], 2);
                }
            }
        }
    ...
    return (
        <div className='w-full chartView-container'>           
            <div className='charResponsive'>
            <ResponsiveContainer width='100%' height={300}>
              ...
              >
                  ...
                  <YAxis 
                      label={{ value: inputData.selectedFilter, angle: -90, position: 'insideLeft' }} 
                      axisLine={false}
                      tickFormatter={(value) => {
                          if (value >= 1000000) {
                  ...
                  <Legend layout='horizontal' verticalAlign='top' align='right' wrapperStyle={{ paddingTop: '10px' }} />
                  
                  <Bar legendType='circle' name={`Extra ${inputData.selectedFilter}`} dataKey='EXTRA' stackId='a' fill='#1d7c2b' />
                  ...
                  {inputData.selectedFilter === 'Hours' && (
                      <>
                          <Line legendType='circle' name='Actual Capacity' dataKey='PRODUCTIVE_CAPACITY' stroke='#FF0000' dot={false} />
                          <Line legendType='circle' name='Optimal Capacity' dataKey='OPTIMAL_CAPACITY' stroke='#8F398F' dot={false} />
                          <Line legendType='circle' name='Max Capacity' dataKey='MAX_CAPACITY' stroke='#7f7e81' dot={false} />
                          <Line legendType='circle' name='Min Capacity' dataKey='MIN_CAPACITY' stroke='#000000' dot={false} />
                      </>
                  )}
