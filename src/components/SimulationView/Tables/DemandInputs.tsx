...
import { LdLoading, LdTypo, LdButton ,LdModal,LdCheckbox,LdIcon } from "@emdgroup-liquid/liquid/dist/react";
...
const allColumns = ["IOP_BRAND", "IOP_STRENGTH", "IOP_PRODUCT", "IOP_DESCR", "LINES_NUMBER", "UNIT_PER_PALLET", "OVERHEAD_COST_BY_UNIT", "PRODRATE", "BATCH_SIZE","HOURS", "QTY", "BATCHES", "PALLETS", "ABSORPTION"];
const initialFilterColumns = ["IOP_PRODUCT", "IOP_DESCR", "LINES_NUMBER", "PRODRATE", "BATCH_SIZE", "HOURS", "QTY", "BATCHES"];
const allRateUnitColums = ["IOP_BRAND", "IOP_STRENGTH", "IOP_PRODUCT", "IOP_DESCR", "LINES_NUMBER", "UNIT_PER_PALLET", "OVERHEAD_COST_BY_UNIT", "PRODRATE", "BATCH_SIZE"];
...
const DemandInputTable: React.FC<DemandInputTableProps> = ( 
    ...
    const duplicateRow = useRef<{lineKey: string; line: string; lineIndex: number } | null>(null);
    ...
    const showTableColumns = allRateUnitColums.filter(column => filterColumns.includes(column));
    const showUnitsColumns = allUnitsColumns.filter(column => filterColumns.includes(column));
    ...
    const { filterDemandInputsData, uniqueValues } = useMemo(() => {
        ...
        const result: Record<string, string[]> = Object.fromEntries(
            Object.entries(uniqueData).map(([field, values]) => [field, Array.from(values)])
        );
    ...
    const rateEvent = useCallback((linekey:string , productIndex:number, inputData:any, e:any, rate:string, target_unit:string, multiply=false, round_two=false, ceil=false) => {
        ...
        for (const key in inputData){
            if (key.includes(target_unit) && !key.includes("ORIGINAL")){
                ...
                unitValue = parseFloatNoNull(round_two? parseFloat(unitValue.toFixed(2)): (ceil? Math.ceil(unitValue): Math.round(unitValue)));
                ...
                if (showUnitsColumns.includes(target_unit)){
                    ...
                    const unitInput = document.getElementById(`input-box-${key}-${linekey}-${productIndex}`);
                    unitInput.value = unitValue;     

                    findRowTotalValue(key,`input-box-${key}`,`total-${key}`);
                    let originalValueUnit = inputData[`${date}_ORIGINAL_${unitKeyName}`][productIndex];
    ...
    const unitEvent = useCallback((line: string, inputData:any, key: string, key_index: number, e: any) => {
        const date = key.split("_").slice(0, 3).join("_");
        ...
        saveRateUnitsContent(line,inputData,key_index,key,parseFloat(e.target.value));

        let unitBox = document.getElementById(`input-box-${date}_${unit}-${line}-${key_index}`);  
        
        findRowTotalValue(`${date}_${unit}`,`input-box-${date}_${unit}`,`total-${key}`);
		...
		const units_to_adjust = ["HOURS", "QTY", "BATCHES", "PALLETS", "ABSORPTION"].filter(x => x !== unit);
        ...
        for (const unit_adj of units_to_adjust){
            ...
            switch(unit_adj){
                ...
                case "HOURS":
                    ...
                    // const prodRate = document.getElementById(`input-prodrate-${line}-${key_index}`).value
                    const prodRate =  currentData[line].PRODRATE[key_index];
                    unitValue = parseIntNoNull(Math.round(prodRate === 0 ? 0 : qty_value / prodRate));
                    break;
                case "BATCHES":
                    // const batchSize = SlateFunctions.getState().input_output.BATCH_SIZE[index];
                    // const batchSize = document.getElementById(`input-batch-size-${line}-${key_index}`).value
                    const batchSize = currentData[line].BATCH_SIZE[key_index];
                    unitValue = parseFloatNoNull(parseFloat((batchSize === 0 ? 0 : qty_value / batchSize).toFixed(2)));
                    break;
                case "PALLETS":
                    ...
                    unitValue = parseIntNoNull(Math.ceil(unit_per_pallet === 0 ? 0 : qty_value / unit_per_pallet));
                    break;
                case "ABSORPTION":
                    ...
                    const overhead_cost_by_unit = currentData[line].OVERHEAD_COST_BY_UNIT[key_index];
                    unitValue = parseIntNoNull(Math.round(qty_value * overhead_cost_by_unit));
                    break;
                default:
                    unitValue = 0;
            }
            ...
            saveRateUnitsContent(line,inputData,key_index,`${date}_${unit_adj}`,unitValue);
            ...
            if (showUnitsColumns.includes(unit_adj)) {
                ...
                let unitBox = document.getElementById(`input-box-${date}_${unit_adj}-${line}-${key_index}`);
            
                let original_value_unit_adj = inputData[`${date}_ORIGINAL_${unit_adj}`][key_index];
                ...
                findRowTotalValue(`${date}_${unit_adj}`,`input-box-${date}_${unit_adj}`,`total-${date}_${unit_adj}`);
    ...
    const getCellColor = useCallback((current_value: number, original_value: number) => {
    ...
    const saveEditedContent = (lineKey, lineData, productIndex, key_string, event) => {
        ...
        const existingIndex = editedInputDataRef.current.findIndex(entry => 
    ...
    const saveRateUnitsContent = (lineKey, lineData, productIndex, key_string, value) => {
        // Extract date and unit from key_string
        const parts = key_string.split("_");
        const unit_type = parts.pop(); 
        const dateParts = parts.join("_").split("_");
    ...
    const saveRateContent = (lineKey, product, productIndex, key_string, event) => {
        ...
        const total_unit_adj = parseFloatNoNull(
            selectedLines.reduce((acc, lineKey) => {
                ...
                if (Array.isArray(lineValue)) {
                    return acc + lineValue.reduce((innerAcc, value) => innerAcc + parseFloatNoNull(value), 0); // Sum the values for each lineKey
                }
    ...
    const adjustSticky = () => {
        ...
        rows.forEach((row: HTMLTableRowElement) => {
            const stickyColumns = row.querySelectorAll("th.sticky, td.sticky") as NodeListOf<HTMLTableCellElement>;
            ...
            stickyColumns.forEach((cell: HTMLTableCellElement, index: number) => {
                ...
                rows.forEach((row: HTMLTableRowElement) => {
                    ...
                    if (currentCell) { // Check if currentCell is defined
    ...
    const addNewSkuAPIcall = async () => {
        if (selectedNewSkuLine!="") {
            ...
            try {
                ...
                display_notification('error', 'An error occurred while adding the new SKU. Please try again.');
                return null; // Return null on error
            }          
        } else {
            display_notification('alert', 'Your request to add New SKU API. Please try again.');
        ...
        if (duplicateRow.current !== null && duplicateRow.current.line) {
            ...
            if (lineData) {
                const postData = {
                    ...
                    "unit_per_pallet": lineData.UNIT_PER_PALLET[duplicateRow.current.lineIndex],
                    "overhead_cost_by_unit": lineData.OVERHEAD_COST_BY_UNIT[duplicateRow.current.lineIndex]
                };
    ...
    const saveDemandInput = async () => {
        ...
        if (editedInputDataRef.current !== null && editedInputDataRef.current.length > 0) {
            try {
                const res = await axios.post(`${apiUrl}/simulation/${SimulationID}/planDetails/edits?simulation_id=${SimulationID}`, 
                ...
                if (res && res.data) { 
                    ...
                    display_notification('alert', 'We are unable to process your request at this moment. Please refresh the page.');
                }
            } catch (err) {
                display_notification('alert', 'We are unable to process your request at this moment. Please refresh the page.');
            }          
        } else {
            display_notification('alert', 'No data to save. Please modify unit value data before saving.');
        }
    };
    
    const handleCheckboxChange = useCallback((lineKey,lineData, productIndex, event) => {
        const currentCheckbox = event.target;
    
        if (currentCheckbox.checked) {
            const selectedRow = { lineKey : lineKey , line: lineData, lineIndex: productIndex };
    ...
    const Row = useCallback(({ lineKey, lineData }: { lineKey: string; lineData: RowData }) => {
            ...
            <React.Fragment>
                {lineData.IOP_PRODUCT.map((product, productIndex) => (
                    <tr key={`${lineKey}-${productIndex}`}>
                        <td>
                            <input type='checkbox' onChange={(e) => handleCheckboxChange(lineKey,lineData, productIndex, e)} value={lineKey} />
                        </td>
                        <td >{lineKey}</td>
                        {showTableColumns.includes("IOP_BRAND") && <td >{lineData.IOP_BRAND[productIndex]}</td>}
                        {showTableColumns.includes("IOP_STRENGTH") && <td >{lineData.IOP_STRENGTH[productIndex]}</td>}
                        {showTableColumns.includes("IOP_PRODUCT") && <td >{product}</td>}
                        {showTableColumns.includes("IOP_DESCR") && <td >{lineData.IOP_DESCR[productIndex]}</td>}
                        {showTableColumns.includes("LINES_NUMBER") && <td >{lineData.LINES_NUMBER[productIndex]}</td>}
                        ...
                        {showTableColumns.includes("OVERHEAD_COST_BY_UNIT") && (
                            <td style={{ backgroundColor: 'white' }} >
                                <input 
                                    ...
                                    defaultValue={lineData.OVERHEAD_COST_BY_UNIT[productIndex] || 0} 
                                    tabIndex="-1"                                />
                            </td>
                        )}
                        {showTableColumns.includes("PRODRATE") && (
                            <td style={{ backgroundColor: 'white' }} >
                                <input 
                                    ...
                                    onChange={(e) => rateEvent(lineKey,productIndex, lineData ,e, "PRODRATE", "HOURS", false, false, false)}
                                    onBlur={(e) => saveRateContent(lineKey,lineData['IOP_PRODUCT'][productIndex], productIndex, "prodrate",  e)}
                                />
                            </td>
                        )}
                        {showTableColumns.includes("BATCH_SIZE") && (
                            <td style={{ backgroundColor: 'white' }} >
                                <input 
                                    ...
                                    onChange={(e) => rateEvent(lineKey,productIndex, lineData ,e, "BATCH_SIZE", "BATCHES", false, false, false)}
                                />
                            </td>
                        )}
                        {Object.keys(demandLoadAgg).map((agglineKey) => {
                            ...
                            return loadLineData.DATE.map((date, dateIndex) => {
                                ...
                                return (
                                    <React.Fragment key={`${agglineKey}-${productIndex}-${dateIndex}`}>
                                        {["HOURS", "QTY", "BATCHES", "PALLETS", "ABSORPTION"].map((unit) =>
                                            showUnitsColumns.includes(unit) ? (() => {
                                                const originalVal = lineData[`${formattedDate}_ORIGINAL_${unit}`]?.[productIndex] ?? 0;
                                                const editValArray = filterDemandInputsDataRef.current[lineKey]?.[`${formattedDate}_${unit}`] || [];
                                                const editVal = editValArray[productIndex]; 
                                                const defaultVal = editVal ?? lineData[`${formattedDate}_${unit}`]?.[productIndex] ?? 0;
                                                return (
                                                    <td key={`${unit}-${agglineKey}-${productIndex}-${dateIndex}`} style={{
                                                        backgroundColor: getCellColor(defaultVal, originalVal)
                                                    }}>
                                                        <input
                                                            type="number"
                                                            ...
                                                            style={{ backgroundColor: getCellColor(defaultVal, originalVal) }}
                                                            defaultValue={defaultVal}
                                                            readOnly={["PALLETS", "ABSORPTION"].includes(unit)} // Set readOnly for specific units
                                                            onChange={["PALLETS", "ABSORPTION"].includes(unit) ? undefined : (e) => unitEvent(lineKey, lineData, `${formattedDate}_${unit}`, productIndex, e)}
                    ...
                    {Object.keys(demandLoadAgg).length > 0 ? (
                        <div className='w-full'>
                            <div className="flex items-center w-full mt-2 space-x-2 justify-end">
                                <LdButton size="sm" onClick={onDemandSave}  className="simulationView-header-button">Update chart & Table</LdButton>
                                ...
                                <LdButton size="sm" onClick={saveDemandInput} className="simulationView-header-button">Save Data</LdButton>
                                <LdButton size="sm"  onClick={duplicateRowApiCall} className="simulationView-header-button">
                                    Duplicate Row
                                </LdButton>
                            ...
                            <div id='demandInputTbl' className='mt-4 max-h-[32rem] overflow-y-scroll overflow-x-scroll hide-scrollbar'>
                                <table id='products-table'>
                                    <thead>
                                        {/* First table tr start */}
                                        {showUnitsColumns && showUnitsColumns.length > 0 ? (
                                            <tr>
                                                <th style={{ width:'40px'}} colSpan={showTableColumns.length + 2}></th>
                                                {Object.keys(demandLoadAgg).map((lineKey, index) => {
                                                    const lineData = demandLoadAgg[lineKey];
                                                    return lineData.DATE.map((date, dateIndex) => (
                                                        <th className='dm-custom-th' key={`date-header-${date}-${dateIndex}`} colSpan={showUnitsColumns.length}>
                                                            {date.replace(/_/g, '-')}                                                        
                                                        </th>
                                                    ));
                                                })}
                                        ...
                                        <tr>
                                            ...
                                            <th>
                                                LINE <span className="filter-icon">▼</span><br />
                                                <div>
                                                    <select onChange={(e) => filterTable(e,'LINE')} className="flex-none filter-dropdown hidden" placeholder='ALL'>
                                                ...
                                                <th  key={`header-${col}-${index}`}>
                                                    {col === "LINES_NUMBER" || col === "IOP_BRAND" || col === "IOP_PRODUCT" || col === "IOP_STRENGTH" ? (
                                                        <>
                                                            ...
                                                            <span onClick={(e) => toggleDropdown(e.currentTarget.nextElementSibling)} className="filter-icon">▼</span>
                                                            <select onChange={(e) => filterTable(e,col)} className="flex-none filter-dropdown hidden" placeholder='ALL'>
                                                                <option key='' value=''>ALL</option>
                                        ...
                                        {showUnitsColumns && showUnitsColumns.length > 0 ? (
                                            <tr>
                                                <th className='dm-custom-total' colSpan={showTableColumns.length + 2}></th>
                                                {Object.keys(demandLoadAgg).map((lineKey, index) => {
                                                    const lineData = demandLoadAgg[lineKey];
                                                    return lineData.DATE.map((date, dateIndex) => {
                                                        const formattedDate = date.replace(/-/g, '_'); // Change date format
                                                        return (
                                                            <React.Fragment key={`units-header-${formattedDate}-${dateIndex}`}>
                                                                {showUnitsColumns.includes("HOURS") && ( 
                                                                    <th key={`hours-total-header-${formattedDate}`}>
                                                                        <span datatotal={lineData.HOURS[dateIndex]} className='head_total' id={`total-${formattedDate}_HOURS`}>
                                                                ...
                                                                {showUnitsColumns.includes("QTY") && (
                                                                    <th key={`qty-total-header-${formattedDate}`}>
                                                                        <span datatotal={lineData.QTY[dateIndex]} className='head_total' id={`total-${formattedDate}_QTY`}>
                                                                ...
                                                                {showUnitsColumns.includes("BATCHES") && (
                                                                    <th key={`batches-total-header-${formattedDate}`}>
                                                                        <span datatotal={lineData.BATCHES[dateIndex]} className='head_total' id={`total-${formattedDate}_BATCHES`}>
                                                                            {formatWithCommas(lineData.BATCHES[dateIndex])}
                                                                        </span>
                                                                    </th>
                                                                )}
                                                                {showUnitsColumns.includes("PALLETS") && ( 
                                                                    <th key={`pallets-total-header-${formattedDate}`}>
                                                                        <span datatotal={lineData.PALLETS[dateIndex]} className='head_total' id={`total-${formattedDate}_PALLETS`}>
                                                                            {formatWithCommas(lineData.PALLETS[dateIndex])}
                                                                        </span>
                                                                    </th>
                                                                )}
                                                                {showUnitsColumns.includes("ABSORPTION") && (
                                                                    <th key={`absorption-total-header-${formattedDate}`}>
                                                                        <span datatotal={lineData.ABSORPTION[dateIndex]} className='head_total' id={`total-${formattedDate}_ABSORPTION`}>
                                                                            {formatWithCommas(lineData.ABSORPTION[dateIndex])}
                                                                        </span>
                                                                    </th>
                                                                )}
                                                            </React.Fragment>
                                    ...
                                    <tbody>
                                        ...
                                        {Object.keys(filterDemandInputsData).map((lineKey) => {
                                            ...
                                            return <Row key={lineKey} lineKey={lineKey} lineData={lineData} />;
                                        })}
                                        {/* ------------------------------Dynamic tbody end ------------ */}
                                    </tbody>
                                </table>
