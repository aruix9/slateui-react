...
const ComparisonTable: React.FC<ComparisonTableProps> = ({ inputData }) => {
    ...
    const createTableRows = (inputData: Record<string, any>) => {
        ...
        if (tableBody) {
            ...
            const exercises_count = inputData.EXERCISE.filter((value:any, index:any, self:any) => 
            ...
            inputData.PROJECTION.forEach((projection: string, index: number) => {
                ...
                if (index % exercises_count === 0) {
                    if (index !== 0) { cell.style.borderTop = "3px solid #ddd"; }
                    cell.textContent = projection;
                }
                row.appendChild(cell); // Add the merged cell

                // Add cells for the other columns (EXERCISE, VALUE)
                const keys = Object.keys(inputData).filter(x => x !== "PROJECTION" && x !== "by_scenarios");
                keys.forEach(key => {
                    const cell = document.createElement("td");
                    if (key !== "EXERCISE") {
                        if (metrics[index] !== "D/C RATIO") {
                            cell.textContent = parseInt(inputData[key][index]).toLocaleString("en-us");
                        } else {
                            cell.textContent = `${parseInt(inputData[key][index]).toLocaleString("en-us")} %`;
                        }
                    } else {
                    ...
                    if (index % exercises_count === 0 && index !== 0) { cell.style.borderTop = "3px solid #ddd"; }
                    if (key === "EXERCISE") {
                        cell.className = "sticky";
                        cell.style.textAlign = "left";
                    }
                    ...
                    if (inputData["by_scenarios"].map((x:any) => x.replace(/ /g, "_")).includes(inputData["EXERCISE"][index])) {
                        cell.style.backgroundColor = ["EXERCISE", "TOTALS"].includes(key) ? "#D3D8DE" : "#DCE0E5";
                    }
                    if (metrics[index] === "D/C RATIO" && key !== "EXERCISE") {
                        const value = inputData[key][index];
                        cell.style.backgroundColor = (value < 70) ? "rgba(251, 179, 96, 1)" : (value < 90) ? "rgba(114, 202, 155, 1)" : "rgba(250, 153, 156, 1)";
                    }
